...
(function (wp) {
	...
	function removePaginationUiBits() {
		...
		const arrowGroup = document.querySelector(
			'[role="radiogroup"][aria-label="Arrow"]'
		);

		if (arrowGroup) {
			const arrowButton = arrowGroup.querySelector(
				'button[role="radio"][data-value="arrow"], button[aria-label="Arrow"]'
			);
		...
		const possibleLabelGroups = Array.from(
			...
			return (
				...
				label.includes('text') ||
				text.includes('show label') ||
				text.includes('hide label') ||
				text.includes('display label') ||
				text.includes('show text') ||
	...
	const withPaginationUiSimplified = createHigherOrderComponent(
		(BlockEdit) => (props) => {
			useEffect(() => {
	...
	addFilter(
		'editor.BlockEdit',
		'pitchfork/query-pagination-simplify-ui',
		withPaginationUiSimplified
	);
})(window.wp);
